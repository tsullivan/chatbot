...
export class WaterfallLocation extends Location {
  ...
  public getDescription(_game: Adventure) {
    const lns = [
      s`It is so magical here.`,
      s`This waterfall is at the bottom of a giant mountain and a bunch of
        ...
        it.`,
      s`There's a climbing rope that leads to a really big circle. You're not
      sure what it is, and you don't feel strong enough to climb up right now.`,
      s`There's water spraying everywhere!`,
    ];
    return p(lns);
  }
  ...
  public setLocationKeywords(_game: Adventure) {
    ...
    this.addKeyword('MOUNTAIN', `Try to climb up the really tall mountain.`, () => {
      if (this.gotSprayed) {
        ...
        const px = s`With the magical strength gained from the waterfall
          spray, you climb up the very tall mountain.`;
        return this.followExit(UP, px);
      } else {
        const ps = [
          s`You try to hike the trail going straight up the really tall
    ...
    this.addKeyword('ROPE', `Try to climb the rope`, () => {
      if (this.gotSprayed) {
        this.gotSprayed = false;
        const px = s`With the magical strength gained by the waterfall spray,
          you climb like Spider-Man right up the rope to the giant circle.`;
        return this.followExit(WEST, px);
      } else {
        const ps = [
          s`You try to climb the rope up to the giant circle, but you're too
            weak right now!`,
          'LOSE A POINT',
        ];
        return new KeywordResponse({
    ...
    this.addKeyword('GET_SPRAYED', `Allow yourself to get sprayed by the water`, () => {
      ...
      return new KeywordResponse({
        text: s`You jump around in the waterfall spray for a bit. It feels so
    ...
    this.addKeyword(
      ...
      () => {
        if (this.gotSprayed) {
          return new KeywordResponse({
            text: s`You have recently been sprayed by some of the magical droplets of
            the waterfall. You feel magically powerful!`,
          });
        } else {
          return new KeywordResponse({
            text: s`You have dodged all the magical spray from the water, and
