...
export class AppleShedLocation extends Location {
  ...
  public getDescription(_game: Adventure) {
    const lns = [
      s`You're in a giant, giant red sphere in the trees. This is an Apple
    ...
    if (this.numApples > 0) {
      lns.push(
        s`There's no one in the store, but there's a man standing on a tree
          branch out back. He looks inside a window and sees you, then he comes
          in and says a friendly hello.`,
        s`"Oh, a customer! Would you like to buy an apple?" he says.`
      );
  ...
  public setLocationKeywords(game: Adventure) {
    ...
    if (!game.inInventory(APPLES) || !game.inInventory(YOGURT)) {
      pxEx = s`"Thanks for coming!" says the business man. "I still have a few items left for sale if
    ...
    this.addKeyword('LOOK', `Look at the contents of the Apple Store`, () => {
      ...
      if (this.numApples > 0) {
        ...
        this.hasYogurt = true;
        resp = new KeywordResponse({
          text: s`There are no apples on the shelves. There is nothing in the
    ...
    if (!game.inInventory(APPLES)) {
      this.addKeyword(
        ...
        () => {
          ...
          const lns = [
            s`"Oh, you'd like to own one of these delicious apples?" says the man. "Well, you can have one. It costs $1,000."`,
            s`"I have no money!" You tell him.`,
            s`"Oh, well, you can still have one apple, for free." he says.`,
            s`"THANKS!" you say.`,
            s`"Actually, you can have all of the apples for free." he says.`,
            'GAIN 10 POINTS',
          ];
      ...
      this.addKeyword('BUY_YOGURT', `Purchase the yogurt with ghosts in it.`, () => {
        ...
        const lns = [
          s`"Oh, you'd like to own this yogurt?" says the man. "Well, you can have it for free, because it has ghosts in it."`,
          s`"GHOSTS!?" You say.`,
          s`"Yes, ghosts." he says.`,
          s`"THANKS FOR THE GHOSTGURT!" you say.`,
          s`"By the way, ghosts dislike being on the sun." he says.`,
          'GAIN 6 POINTS',
        ];
        return new KeywordResponse({
          changeScore: 6,
