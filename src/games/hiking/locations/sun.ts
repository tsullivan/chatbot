...
export class TheSunLocation extends Location {
  ...
  public getDescription(_game: Adventure) {
    return s`It's so hot on the Sun! Fear not, it's safe inside the sun ship.
  ...
  public setLocationKeywords(game: Adventure) {
    this.addKeyword('BACK', `Go back to Earth`, () => {
      const px = s`KSHOOOOM!! Back down to Earth you go in the sun ship. Hope
      ...
      () => {
        if (this.hasGhosts) {
          return new KeywordResponse({
            text: s`On the surface of the sun, there is melted yogurt and the
            remains of exploded ghosts.`,
          });
        } else {
          return new KeywordResponse({
            text: s`There's not much to say about the sun at the moment. It's
    ...
    if (game.inInventory(YOGURT)) {
      this.addKeyword('THROW_YOGURT', `Throw the ghost yogurt onto the sun`, () => {
        ...
        this.hasGhosts = true;
        const lns = [
          s`You reach for the ghost yogurt in your pocket, open the hatch of
            the sunship, and with all your strength, you mightily hurl the yogurt
            onto the surface of the sun.`,
          s`The yogurt quickly melts, releasing the dreadful ghosts. The
            ghosts begin to moan and start turning into smoke.`,
          s`One of the ghosts looks at you and says, "Thank you for releasing
            us! Our sprits are free now, and we can join our ancestors in ghost heaven."`,
          s`Then all of the ghosts explode`,
          'GAIN 20 POINTS',
        ];
        return new KeywordResponse({
          changeScore: 20,
