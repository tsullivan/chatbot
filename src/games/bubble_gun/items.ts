...
const bubbleGunCombinings = (item: Item, combinedSet: CombinedSet) => {
  ...
  if (combinedSet.has(BATTERY_AA)) {
    ...
    lns.push(
      s`You slide off the cover to the bubble gun, pop in the AA batteries, and put the cover back on. It's nice and snug with the water seal that will
    ...
    lns.push(
      s`You open up the cap on the bubble gun and fill up its soap container with your liquid soap. Glug glug glug.`
    );
  }
  if (item.isComplete()) {
...
const throwieCombinings = (item: Item, combinedSet: CombinedSet) => {
  ...
  if (combinedSet.has(BATTERY_LR41)) {
    item.setDescription(
      s`LED throwie that has batteries, but you can't do anything else with it.`
    );
    lns.push(
      s`The batteries go right on in to the LED's battery holder. Nice and simple.`
    );
  }
  if (item.isComplete()) {
...
const getCombiningResponse = (item: Item, combinedSet: CombinedSet) => {
  ...
  if (id === BUBBLE_GUN) {
    ...
    return new KeywordResponse({ text: p(lns) });
  } else if (id === LED) {
...
export function getItems(game: Adventure) {
  ...
  return {
    batteryAaItem: itemFactory(BATTERY_AA, {
      description: s`The two AA batteries are the standard 1.5V medium-size batteries. They'd fit nicely into the battery holder of a medium-size piece of electronics.`,
      name: 'Medium-size AA Batteries.',
      setActions: ({ setTakeable }) => {
        setTakeable({
          fn: () =>
            new KeywordResponse({
              text: s`The AA batteries are now yours!`,
            }),
    ...
    batteryLr41Item: itemFactory(BATTERY_LR41, {
      description: s`The two LR41 batteries are the standard 1.5V button size batteries. They'd fit nicely into the battery holder of a very small piece of electronics.`,
      name: 'Small LR41 Batteries.',
      setActions: ({ setTakeable }) => {
        setTakeable({
          fn: () =>
            new KeywordResponse({
              text: s`The LR41 batteries are now yours!`,
            }),
    ...
    bubbleGunItem: itemFactory(BUBBLE_GUN, {
      description: s`It's a bubble gun for gunning out bubbles. If you have soap and two AA batteries, it can fulfill the purpose of its existence.`,
      name: 'Bubble Gun',
      setActions: ({ setTakeable, setCombinables }, _item: Item) => {
        setTakeable({
          fn: () =>
            new KeywordResponse({
              text: s`The bubble gun is yours, as it was meant to be!`,
            }),
        ...
        setCombinables([
          ...
          },
          {
            ...
            keywordDescription: s`Put the soap into the soap container of the bubble gun.`,
          },
        ]);
      },
    }),
    ladderItem: itemFactory(LADDER, {
      description: s`This is one sturdy ladder. The bottom part of the ladder is on the ground, and the top part of the ladder leads to the tree fort.`,
      name: 'Ladder',
    }),
    ledItem: itemFactory(LED, {
      description: s`A semiconductor diode which glows when a voltage is applied. It's an LED connected to a circuit board for making it flashy, and battery holder for LR41s.`,
      ...
      setActions: ({ setTakeable, setCombinables }) => {
        setTakeable({
          fn: () =>
            new KeywordResponse({
              text: p([
                s`The LED is now yours, and yours alone.`,
                s`Hmm. Something about this LED seems magical.`,
              ]),
            }),
          keyword: 'TAKE_LED',
    ...
    soapItem: itemFactory(SOAP, {
      description: s`This small bottle of liquid soap has more than enough liquid for a bubble gun to shoot out plenty of bubbles.`,
      name: 'Bottle of liquid soap',
      setActions: ({ setTakeable }) => {
        setTakeable({
          fn: () =>
            new KeywordResponse({
              text: s`Did you forget that I said everything in this town is FREE!? Oh well. You wanted the soap, you got it. It's your problem now.`,
            }),
