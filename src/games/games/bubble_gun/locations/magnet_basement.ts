...
export class MagnetBasementLocation extends Location {
  ...
  public getDescription(game: AdventureGame) {
    const machineConditional = [
      snl`It is a very well-lit and clean place - it feels sterile.`,
  ...
  public setLocationKeywords(game: AdventureGame) {
    ...
    this.addKeyword(
      ...
      () => {
        ...
        The button is labeled \`STATUS_CYCLE\`. There is another slightly larger screen on the control panel, which is a bird's-eye view of Bubble Gun World. It
        shows targets on the Playground and the Bridge.`;
        return new KeywordResponse({ text });
      },
    );

    if (this.machineUncovered) {
        ...
        switch (this.machineStatus) {
          case STAT_EXPOSED:
            machineText = parajoin([
              snl`The bright florescent lights suddently shut off. Red alerting lights turn on and paint the large room with red light and long shadows.`,
